...
class Visualizer:
    def __init__(self, limits=[-10,10,-10,10], x0=np.zeros((3,1)),
        ...
        info_vec = np.linalg.inv(sigma0) @ xhat0
        self.info_vec_hist = [info_vec.flatten()]
    ...
    def update(self, t, true_pose, est_pose, covariance, zhat, info_vec):
        ...
        self.info_vec_hist.append(info_vec.flatten())
    ...
    def plotHistory(self):
        ...
        self.info_vec_hist = np.array(self.info_vec_hist)
        fig4, axes4 = plt.subplots(3,1, sharex=True)
        axes4[0].plot(self.time_hist, self.info_vec_hist[:,0])
        axes4[0].set_title('Information Vector $\\xi$ Plots')
        axes4[0].set_ylabel('$\\xi_x$')

        axes4[1].plot(self.time_hist, self.info_vec_hist[:,1])#, label='y')
        axes4[1].set_ylabel('$\\xi_y$')

        axes4[2].plot(self.time_hist, self.info_vec_hist[:,2])#, label='$\\theta$')
        axes4[2].set_ylabel('$\\xi_\\theta$')
